...
class AuthService:
    ...
    def store_oauth_token(self, tenant_id: UUID, oauth_result: OAuthResult) -> AdToken:
        ...
        if oauth_result.platform == Platform.facebook and oauth_result.raw.get("pages"):
            self._store_facebook_page_tokens(tenant_id, oauth_result.raw["pages"])

        self.db.commit()
        return token

    def _store_facebook_page_tokens(self, tenant_id: UUID, pages: list) -> None:
        ...
        for page in pages:
            ...
            existing_page_token = self.ad_token_repo.get_active_token(
                tenant_id, Platform.facebook, f"page_{page_id}"
            )
    ...
    def get_facebook_api_client(self, tenant_id: UUID, account_ref: str = None) -> Optional[FacebookAPIClient]:
        """Get Facebook API client with decrypted token"""
        token = self.get_decrypted_token(tenant_id, Platform.facebook, account_ref)
        if token:
            return FacebookAPIClient(token)
        return None

    def get_facebook_page_tokens(self, tenant_id: UUID) -> list[dict]:
        """Get all Facebook page tokens for a tenant"""
        tokens = self.ad_token_repo.get_by_platform(tenant_id, Platform.facebook)
        page_tokens = []
    ...
    def get_tiktok_api_client(self, tenant_id: UUID, account_ref: str = None):
        """Get TikTok API client with decrypted token"""
        from app.integrations.tiktok import TikTokAPIClient

        token = self.get_decrypted_token(tenant_id, Platform.tiktok, account_ref)
        if token:
            return TikTokAPIClient(token)
        return None

    def get_tiktok_creator_info(self, tenant_id: UUID, account_ref: str = None) -> dict:
        """Get TikTok creator information for a tenant"""
        tokens = self.ad_token_repo.get_by_platform(tenant_id, Platform.tiktok)
